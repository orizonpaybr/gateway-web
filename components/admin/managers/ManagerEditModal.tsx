...
export const ManagerEditModal = memo(
  ({ open, onClose, manager, onSubmit, isSaving }: ManagerEditModalProps) => {
    ...
    const validateForm = useCallback((): boolean => {
      ...
      for (const rule of validationRules) {
        const fieldValue = formData[
          rule.field as keyof typeof formData
        ] as string

        if (
          rule.condition &&
          !rule.condition(formData as Record<string, unknown>)
        ) {
          continue
        }
